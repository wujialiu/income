...
MIDCAP400_LIST_FILE = "sp400.txt"
NASDAQ100_LIST_FILE = "nasdaq100.txt"
DOW30_LIST_FILE = "dow30.txt"
...
def fetch_midcap400_tickers() -> List[str]:
    return fetch_index_tickers(WIKIPEDIA_MIDCAP400_URL, "S&P MidCap 400")
...
def read_ticker_list(filename: str) -> List[str]:
    path = Path(filename)
    if not path.exists():
        logging.info("No %s file found. Continuing without extra tickers.", filename)
        return []

    tickers: List[str] = []
    for line in path.read_text(encoding="utf-8").splitlines():
        cleaned_line = line.partition("#")[0].replace(",", " ")
        tickers.extend(symbol for symbol in cleaned_line.split() if symbol.strip())

    normalized = build_ticker_universe(tickers)
    logging.info("Loaded %d extra tickers from %s.", len(normalized), filename)
    return normalized
...
def main() -> None:
    ...
    write_ticker_list(MIDCAP400_LIST_FILE, midcap400)
    write_ticker_list(NASDAQ100_LIST_FILE, nasdaq100)
    write_ticker_list(DOW30_LIST_FILE, dow30)
