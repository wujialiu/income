...
def make_fallback_data(periods: int = 180, base_level: float = 30.0) -> pd.DataFrame:
...
def build_chart(data: pd.DataFrame) -> go.Figure:
    ...
    fig.update_layout(
        title=dict(
            text='Silver Futures Last 180 Days: Price, Bollinger Bands, Gold/Silver Ratio, and RSI',
            x=0.01,
            xanchor='left',
        ),
...
def main() -> None:
    silver_data = load_futures_data('SI=F', fallback_base_level=30.0, period='180d', fallback_periods=180)
    gold_data = load_futures_data('GC=F', fallback_base_level=2400.0, period='180d', fallback_periods=180)
    ...
    try:
        ...
        output_path = Path(__file__).with_name('silver_chart_180.html')
