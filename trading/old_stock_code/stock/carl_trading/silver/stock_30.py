...
def make_fallback_data(periods: int = 120, base_level: float = 100.0) -> pd.DataFrame:
...
def build_chart(data: pd.DataFrame, ticker: str, benchmark: str) -> go.Figure:
    ...
    fig.update_layout(
        title=dict(
            text=f'{ticker} Last 30 Trading Days: Price, Bollinger Bands, Relative Strength, and RSI',
            x=0.01,
            xanchor='left',
        ),
...
def main() -> None:
    if len(sys.argv) != 2:
        raise SystemExit("Usage: python3 stock_30.py <ticker>")

    ticker = sys.argv[1].upper()
    benchmark = 'SPY'

    stock_data = load_market_data(ticker, fallback_base_level=100.0, period='120d', fallback_periods=120)
    benchmark_data = load_market_data(benchmark, fallback_base_level=500.0, period='120d', fallback_periods=120)
    ...
    try:
        ...
        output_path = Path(__file__).with_name(f"stock_chart_30_{ticker.lower()}.html")
